...
func EnsureDefaultProviderProfile(ctx context.Context, repo device.Repository, cfg Config) error {
	...
	profile := device.ProviderProfile{
		...
		MultimodalBaseURL: strings.TrimSpace(cfg.DefaultProviderBaseURL),
		MultimodalAPIKey:  strings.TrimSpace(cfg.DefaultProviderAPIKey),
		MultimodalModel:   strings.TrimSpace(cfg.DefaultMultimodalModel),
