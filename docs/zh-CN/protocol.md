...
如果客户端希望语音通道返回 MCP 候选调用，可以在 `voice_assistant.metadata.mcp_tools`、`voice_assistant.metadata.available_mcp_tools`、`voice_assistant.ui_context.mcp_tools` 或 `voice_assistant.ui_context.available_mcp_tools` 中声明可用工具。AgenSense 只返回 `mcp.call.proposed` 候选，不直接执行 MCP。
...
语音会话根据最终 ASR 文本和客户端声明的 MCP 工具，返回一个候选 MCP 调用。该事件只表示“建议调用”，由客户端或可信网关决定是否确认、执行、改写或忽略。
